...
public:
	...
	SourceMetricsDependency(void *factory_p, destroy_t destroy_p, get_metrics_t get_metrics_p)
...
private:
	...
	uint64_t TakeDelta(std::atomic<uint64_t> &reported, uint64_t current) {
		...
		while (current > previous && !reported.compare_exchange_weak(previous, current, std::memory_order_relaxed,
...
inline SourceMetricsDependency &GetSourceMetricsDependency(const FunctionData *bind_data) {
...
inline void SourceMetricsArrowScan(ClientContext &context, TableFunctionInput &input, DataChunk &output) {
...
inline void SourceMetricsGetMetrics(ClientContext &context, const FunctionData *bind_data, GlobalTableFunctionState &,
...
inline double SourceMetricsProgress(ClientContext &, const FunctionData *bind_data, const GlobalTableFunctionState *) {
